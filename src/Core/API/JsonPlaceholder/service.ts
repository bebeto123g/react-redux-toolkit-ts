...
import { IPost, ITodo, IServicePlaceholderRequestParams } from './interfaces';

export class APIServicePlaceholder {
    static async getTodosPages({
        page = 0,
        pageSize = 10,
    }: IServicePlaceholderRequestParams = {}): Promise<ITodo[]> {
        return APIProvider.get<ITodo[]>(
            `${JSON_PLACEHOLDER_BASE_URL}/todos?_start=${pageSize * page}&_limit=${pageSize}`
        );
    }

    static async getPostsPages({
        page = 0,
        pageSize = 10,
    }: IServicePlaceholderRequestParams = {}): Promise<IPost[]> {
